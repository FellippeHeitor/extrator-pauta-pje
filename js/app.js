...
class PautaExtractorApp {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('select-all-digits').addEventListener('click', () => this.selectAllDigits());
        document.getElementById('select-none-digits').addEventListener('click', () => this.selectNoneDigits());

        // Botão de processar
        document.getElementById('process-btn').addEventListener('click', () => this.processFile());

        // Botões de exportação
        document.getElementById('copy-clipboard-btn').addEventListener('click', () => this.copyToClipboard());
    ...
    showResults(result) {
        // Atualiza resumo
        document.getElementById('periodo-found').textContent = result.periodo || 'Não encontrado';
        document.getElementById('total-found').textContent = result.total.toString();
        document.getElementById('digits-selected').textContent = this.selectedDigits.join(', ');
    ...
    updatePreviewTable(data) {
        ...
        if (data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" style="text-align: center; color: #666;">Nenhum processo encontrado</td></tr>';
            return;
        }
        ...
        if (data.length > 100) {
            const row = document.createElement('tr');
            row.innerHTML = `<td colspan="3" style="text-align: center; color: #666; font-style: italic;">... e mais ${data.length - 100} processos</td>`;
            tbody.appendChild(row);
        }
    }
    ...
    async copyToClipboard() {
        ...
        if (result.success) {
            this.showTemporaryMessage('Dados copiados para a área de transferência!', 'success');
        } else {
    ...
    downloadCSV() {
        ...
        if (result.success) {
            this.showTemporaryMessage(`Arquivo ${result.filename} baixado com sucesso!`, 'success');
        } else {
    ...
    loadSelectedDigits() {
        try {
            const storedDigits = localStorage.getItem('pauta-extractor-selected-digits');
            if (storedDigits) {
    ...
    saveSelectedDigits() {
        try {
            localStorage.setItem('pauta-extractor-selected-digits', JSON.stringify(this.selectedDigits));
        } catch (error) {
    ...
    /**
     * Limpa os dados salvos no localStorage (método utilitário)
     */
    clearStoredData() {
        try {
            localStorage.removeItem('pauta-extractor-selected-digits');
